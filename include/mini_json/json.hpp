...
public:
  ...
  json (std::string init)
      : context (std::move (init)), context_it (context.begin ())
  {
  }
 ...
 */
inline bool
json::parse_unicode (std::string &out)
{
  auto &it = ++context_it;
 ...
 */
inline std::string
json::str_string (std::string_view src)
{
  std::string ret;
...
json::str_object (node &mnode)
{
  ...
  for (auto it = map.begin (); it != map.end ();)
    {
      string->append ("\"").append (str_string (it->first)).append ("\": ");
